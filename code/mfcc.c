...
void PreEmphasis(BYTE * pRawVoice, int iSampleNum, double * pDiffVoice);
double * getTrainFrame(double * pDiffVoice, int iSampleNum, int train_frame_num);
BOOL FrameMultiHamming(double * pDiffVoice, VOICEFRAME * pVoiceFrame, int train_frame_num);
void FrameToMFCC(VOICEFRAME * pVoiceFrame, double  ** MFCC, int train_frame_num);
double HalfFrameEnergy(double * pDiffVoice);
...
void PreEmphasis(BYTE * pRawVoice, int sampleNum, double *pVoice) {
    int i;
    pVoice[0] = pRawVoice[0];
    for (i = 1; i < sampleNum; ++i) {
    	pVoice[i] = pRawVoice[i] - 0.98 * pRawVoice[i-1];
	}
}

double pTranDifft[(FRAME_NUM/2 + 1)*FRAME_LEN];
...
double * getTrainFrame(double *pDiffVoice, int iSampleNum, int train_frame_num) {
    ...
    int iNum;
    int iFrame;
    int iFrameNum = (iSampleNum>>7) - 1;
    double PriorHalfFrame = 0;
    double NextHalfFrame = 0;
    double FrameEnergy = 0;
    double *pTranDiff = NULL;
    double *pSave;
...
BOOL ApplyHamming(double * pDiffVoice, VOICEFRAME * pVoiceFrame, int train_frame_num) {
    int i;
    int iFrame = 0;
    int iPointNum =  FRAME_LEN - 1;

    for (iFrame = 0 ; iFrame < train_frame_num; ++iFrame) {
        pVoiceFrame[iFrame].pCmpx = (COMPX *)mmalloc(sizeof(COMPX)*FRAME_LEN);
        if (!pVoiceFrame[iFrame].pCmpx) {
            printf("mmalloc Frame error");
            while (iFrame > 0) {
        ...
        FFT2(pVoiceFrame[iFrame].pCmpx, FRAME_LEN);
    }
    return TRUE;
}
...
void FrameToMFCC(VOICEFRAME * pVoiceFrame, double  ** MFCC, int train_frame_num) {
    ...
    for (iFrame = 0; iFrame < train_frame_num; ++iFrame) {
        ...
        for (i = 0; i < 129; ++i) {
            for (j = 0; j < D; ++j) {
                if (i < indexM16[j][1] || i > indexM16[j][2]) {
                    continue;
                } else {
...
BOOL voiceToMFCC(BYTE * pRawVoice, int iSampleNum, double ** MFCC, int train_frame_num) {
    ...
    double * pDiff = NULL;
    VOICEFRAME * pVoiceFrame = NULL;

	pDiffVoice = (double *)ccalloc(iSampleNum, sizeof(double));
	assert(pDiffVoice != NULL);

    PreEmphasis(pRawVoice, iSampleNum, pDiffVoice);

    pDiff = getTrainFrame(pDiffVoice, iSampleNum, train_frame_num);
    if (!pDiff) {
		printf("Silent voice!\n");
        free(pDiffVoice);
        return FALSE;
	...
	pVoiceFrame = (VOICEFRAME *)mmalloc(train_frame_num * sizeof(VOICEFRAME));
	assert(pVoiceFrame != NULL);
	ApplyHamming(pDiff, pVoiceFrame, train_frame_num);
    FrameToMFCC(pVoiceFrame, MFCC, train_frame_num);
